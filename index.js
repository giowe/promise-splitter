const splitter = (promises, options, cb) => new Promise(async (resolve, reject) => {
  ...
  const out = [];
  if (verbose) console.log(`Splitting promise list in ${chunkTotal} chunk${chunkTotal > 1 ? 's': ''} of ${chunkSize} promise${chunkSize > 1 ? 's' : ''}, starting from chunk ${startChunk}.`);
  for (let i = (startChunk - 1) * chunkSize; i < l; i+=chunkSize) {
    const chunkIndex = i / chunkSize + 1;
    let retryCount = retryLimit || 1;
    if (verbose) console.log(`chunk ${chunkIndex}`);
    while (retryCount) try {
      ...
      console.log(err);
      console.log(`retry ${retryLimit - retryCount + 1}/${retryLimit}...`);
      retryCount--;
      if (retryCount === 0) {
        return reject(err);
      }
